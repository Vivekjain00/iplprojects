...
//provides add teams an add features on button click by popping in new window
var addteamclicked=()=>{
...
//....................................................
//defining OnClick function to navigate to different card on button click.
...
//settind dynamic grid data in js using local storage for the team card in homepage.
...
for(var i=0;i<teamDetail.length;i++){
    suggestArray.push(teamDetail[i].sName)
    
}
let barSearch = document.querySelector(".search-input");
let input = barSearch.querySelector("input");
let suggestionBox = barSearch.querySelector(".autocom-box");
let icon = barSearch.querySelector(".icon");

//providing filter search on enter key  and icon click according to team name.
...
input.onkeyup = (e)=>{
    ...
    if(userData){
        ...
        barSearch.classList.add("active"); 
        showSuggestions(emptyArray);
        let allList = suggestionBox.querySelectorAll("li");
        for (let i = 0; i < allList.length; i++) {
            
            allList[i].setAttribute("onclick", "currentList(this)");
        }
    }else{
        barSearch.classList.remove("active");
    }
}
function currentList(element){
...
var teamcard=document.getElementById("container_teams")
for(var i=0;i<teamDetail.length;i++){
teamcard.innerHTML+=`
<div    onclick="onCLick('${i}')"    class="minibox">

<img src="${teamDetail[i].teamIcon}" class="mainimage" alt=""/> 
<div class="dataodcard">

  <p class="text1"> ${teamDetail[i].teamFullName}   </p>
